...
__global__ void CalculateTriangles(int m,
    const int* __restrict__ edges, const int* __restrict__ nodes,
...
__global__ void CalculateTriangleSplit(int edge_len,
    ...
    uint64_t* results, const uint64_t* __restrict__ dev_node_index,
    int i, int j) {
    int from = blockDim.x * blockIdx.x + threadIdx.x;
    int step = gridDim.x * blockDim.x;
    uint64_t count = 0;
...
void CudaAssert(cudaError_t status, const char* code, const char* file, int l) {
    ...
    std::cerr << "Cuda error: " << code <<
        ", file " << file << ", line " << l << std::endl;
    exit(1);
}
...
uint64_t GpuForwardSplit(int* edges, int num_nodes, uint64_t num_edges,
    ...
    uint64_t cpu_result;
    std::thread cpu_computation(CalculateTrianglesSplitCPU, edges, host_nodes, (uint64_t)0, node_index[1], std::ref(cpu_result));    
